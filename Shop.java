...
public class Shop {
	...
	public void work () {
		...
		try {
			br = new  BufferedReader(new FileReader("/home/klomszczyk/eclipse-workspace/Lekcjie/shop/src/users.txt"));
			String x = null;
			
			while((x = br.readLine()) != null) {
				String[] line = x.split(" ");
				User u = new User(line[0], line[1], line[2], Integer.parseInt(line[3]));
			...
			if (users.containsKey(login) && users.get(login).getPassword().equals(pass)) {
				...
				if (loggedInUser.getRole() == 0) {
					...
					do {
						...
						System.out.println("[5]. Zmień ilość produktu");
						System.out.println("[10]. Wylogowanie");
						System.out.println("[11]. Wyjście ze sklepu");
						String userChoice = scan.nextLine();
						switch (userChoice) {
							case "1" : {
								System.out.println("Login dla nowego użytkownika: ");
								String newUserLogin = scan.nextLine();
								System.out.println("Password dla nowego użytkownika: ");
								String newUserPass = scan.nextLine();
								System.out.println("Nazwa dla nowego użytkownika: ");
							...
							case "3" : {
								...
								System.out.println("Ilość dla nowego produktu: ");
							...
							case "4" : {
								...
								for (Item item : itemMgr.getItems())
									System.out.println("Produkt [" + i++ + "]: " + item.getName() + ", cena: " + item.getPrice() + ", ilość w magazynie: " + item.getAmount());
								break;
							}
							case "5" : {
								...
								if (scan.hasNextLine()) scan.nextLine();
								System.out.println("Nowa ilość dla produktu: ");
